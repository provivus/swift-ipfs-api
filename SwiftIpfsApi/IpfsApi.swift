...
//        try net.sendTo(baseUrl+"add?r", content: filePaths) {
    ...
    public func add(json: Data, completionHandler: @escaping ([MerkleNode]) -> Void) throws {
        
        //try net.sendTo(<#T##target: String##String#>, content: <#T##Data#>, completionHandler: <#T##(Data) -> Void#>)
        try net.sendTo(baseUrl+"add?stream-channels=true", content: json) {
            //        try net.sendTo(baseUrl+"add?r", content: filePaths) {
            data in
            do {
                /// If there was no data fetched pass an empty dictionary and return.
